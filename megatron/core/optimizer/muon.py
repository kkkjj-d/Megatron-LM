...
class Muon(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=0.02, weight_decay=0.01, momentum=0.95, nesterov=True, ns_steps=5, rank=0, world_size=1):
        # self.rank = backend.comm.local_rank()
        # self.world_size = backend.comm.size()
        # print("environ : ",self.rank,self.world_size)
        defaults = dict(lr=lr, weight_decay=weight_decay, momentum=momentum, nesterov=nesterov, ns_steps=ns_steps)
        params: list[Tensor] = [*params]
        param_groups = []
        for size in {p.numel() for p in params}:
            # print(size)
            b = torch.empty(self.world_size, size, dtype=torch.float32, device="cuda")
